...
def send_mail_template(subject, template_name, context, recipient_list,
    ...
    message_html = render_to_string(template_name, context) #Transforma template em texto

    message_text = striptags(message_html) #Corta as tags html e adiciona ao corpo

    email = EmailMultiAlternatives(
        subject=subject, body=message_text, from_email=from_email, to=recipient_list
    )
    #Possibilita o envio a diversos 'recipientes'
